...
- [ ] README
- [ ] has fail2ban
- [x] made directory for playbooks
...
better differences that are just new features (will add these to reminders):
...
- the AI explicitly set ssh pub key to yes
  - not needed, default is yes
...
- the AI uses libvirt/KVM terraform provider instead of incus
  - this offers more low-level control but then it becomes much harder to manage the firewall because libvirt messes with the iptables a lot, and it can conflict with what i'm trying to do with UFW. on the other hand, there's a very easy way to tell incus to not mess with iptables. that way, UFW has full control, which is more predictable, and therefore more safe.
  - also, incus has great support for OVN and OVS, which use NACLs, which is something i wanna learn, so i think incus is better to stick with
